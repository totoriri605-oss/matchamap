...
현재 맛 데이터는 1~5점이며 메인 필터는 3단계 선택 버튼으로 표시한다. 진하기·쌉싸름함·말차 향은 `1 / 3 / 4`, 단맛은 `2 / 3 / 5`를 기존 이상·이하 필터에 전달한다. 우유맛 데이터와 추천 기능은 유지하지만 메인 필터 패널에는 표시하지 않는다. 세부 동작은 [FEATURES](FEATURES.md)를 따른다.
...
추가 옵션은 체크박스 모양 대신 선택 가능한 pill/chip 버튼으로 표시한다.
